...
exports.createProduct = asyncWrapper(async (req, res) => {
  console.log("[CREATE_PRODUCT] req.user:", req.user);
  console.log("[CREATE_PRODUCT] req.body keys:", Object.keys(req.body));
  console.log("[CREATE_PRODUCT] req.files:", req.files);

  if (!req.user) {
    return res.status(401).json({ success: false, message: "User not authenticated" });
  }

  // Set user ID first, always
